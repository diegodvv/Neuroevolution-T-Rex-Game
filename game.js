...
(function () {
    ...
    Runner.prototype = {
        ...
        /**
         * Calls all the subscribed functions passing the current state as an argument.
         */
        notifyAll: function() {
            this.observers.forEach((observerFunction) => 
                observerFunction(this.state)
            );
        },
                ...
                switch (setting) {
                    ...
                    case 'SPEED_DROP_COEFFICIENT':
                        this.players.forEach((player) => {
                            player.tRex.config[setting] = value;
                        })
                        break;
                    case 'INITIAL_JUMP_VELOCITY':
                        this.players.forEach((player) => {
                            player.tRex.setJumpVelocity(value);
                        })
                        break;
                    case 'SPEED':
                        this.setSpeed(value);
        ...
        update: function () {
            ...
            if (this.playing) {
                ...
                this.players.forEach((player, index) => {
                    ...
                    if (!collision) {
                        player.distanceRan += this.currentSpeed * deltaTime / this.msPerFrame;
        ...
        play: function () {
            if (!this.crashed) {
                ...
                this.paused = false;
                this.players.forEach((player) => {
                    player.tRex.update(0, Trex.status.RUNNING);
                })
                this.time = getTimeStamp();
                this.update();
            }
